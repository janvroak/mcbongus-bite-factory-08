...
const RestaurantCard = ({ restaurant }: RestaurantCardProps) => {
  ...
  return (
    <Link to={`/restaurant/${restaurant.id}`}>
      ...
      >
        {restaurant.featured && (
          <div className="absolute top-3 left-3 z-10 bg-mcbongu-500 text-white text-xs font-medium px-2 py-1 rounded-full">
            Featured
          </div>
        )}
        ...
        <div className="p-4">
          ...
          <div className="mt-3 flex items-center justify-between">
            ...
            <ChevronRight className="h-4 w-4 text-mcbongu-500" />
          </div>
        </div>
      </div>
    </Link>
